...
private fun day1(firstTrace: HashMap<Pair<Int, Int>, Int>, secondTrace: HashMap<Pair<Int, Int>, Int>): Int {
    val min = firstTrace.keys
        .intersect(secondTrace.keys)
        .minBy { pair -> pair.first.absoluteValue + pair.second.absoluteValue }!!

    return min.first.absoluteValue + min.second.absoluteValue
}

private fun day2(firstTrace: HashMap<Pair<Int, Int>, Int>, secondTrace: HashMap<Pair<Int, Int>, Int>): Int {
    return firstTrace.keys
        .intersect(secondTrace.keys)
        .map { intersection -> firstTrace[intersection]!! + secondTrace[intersection]!! }
        .min()!!
}
...
private fun trace(
    ...
    path.split(',').forEach {
        val dir = it[0]
        val mag = it.substring(1, it.length).toInt()
        var dX = 0
        var dY = 0
...
fun main() {
    val firstTrace = trace(firstPath, HashMap())
    val secondTrace = trace(secondPath, HashMap())

    // 403
    println("Day 1: " + day1(firstTrace, secondTrace))

    // 4158
    println("Day 2: " + day2(firstTrace, secondTrace))
}